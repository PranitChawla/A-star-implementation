...
public:
	...
	bool isblockedhor(int i,int j,Mat a);
	bool isblockedver(int i,int j,Mat a);
...
bool PathPlanner::isblockedver (int i,int j,Mat a)
{
	int m,n;
	for (m=i-10;m<=i+10;m++)
	{
...
bool PathPlanner::isblockedhor (int i,int j,Mat a)
{
	int m,n;
	for (m=i-5;m<=i+5;m++)
	{
...
void PathPlanner::tracepath(Point dest,Mat a)
{
    ...
    while(Path.size()>1)
	{
		...
		p2=Path.top();
		float tan;
		tan=(float)(p2.y-p.y)/(p2.x-p.x);
		float theta;
		theta=(float)180*atan(tan)/CV_PI;
		cout<<theta;
		cout<<endl;
		...
		// waitKey(20);
		c=a.clone();
		RotatedRect rRect = RotatedRect(Point2f(p.x,p.y), Size2f(20,10),theta);
		Point2f vertices[4];
	rRect.points(vertices);
	for (int i = 0; i < 4; i++)
    line(c, vertices[i], vertices[(i+1)%4], Scalar(255,255,255));
 		...
 		for (m=i-1;m<=i+1;m++)
 		{
 			for (n=j-1;n<=j+1;n++)
 			{
 				if (isvalid(m,n,a))
 				{
 					...
 					else if ((visit.at<uchar>(m,n)==0)&&isblack(m,n,a)&&(!isblockedhor(m,n,a))&&(!isblockedver(m,n,a)))
 					{
